...
local gui = {
    checkbox = ui_new_checkbox("VISUALS", "Effects", "Kill effect"),
    slider = ui_new_slider("VISUALS", "Effects", "Kill effect duration", 3, 20, 5, true, "s", 0.1),
}


local function on_death(e)
    if ui_get(gui.checkbox) ~= true then
        return
    end

    local attacker = e.attacker
    local attacker_id = client_userid_to_entindex(attacker)
    local local_id = entity_get_local_player()

    if attacker_id == local_id then
        entity_set_prop(local_id, "m_flHealthShotBoostExpirationTime", globals_curtime() + (ui_get(gui.slider) * 0.1))
    end
end


client_set_event_callback("player_death", on_death)
